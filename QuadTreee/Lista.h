...
class Lista
{
  ...
  public:
	...
	void eliminar_sublista(Lista <T> L2); /// Esta malo, no usar
	///***Destructor***
	~Lista();
...
/*
template <class T>
bool Lista<T> :: es_sublista(Lista<T> L2)
{
    Nodo<T> *l2, *aux;
    int cont;
    bool band;
    ...
    while(aux->get_sig() != NULL )
    {
        ...
        if(aux->get_info() == l2->get_info())
        {
            l2=l2->get_sig();
            aux=aux->get_sig();
            cout <<"l2 dentro" << l2->get_info() << endl;
           cout <<"aux  dentro"  << aux->get_info() << endl;
...
template<class T>
void Lista<T> :: eliminar_sublista(Lista <T> L2)
{
    Nodo<T> *l2, *aux;
    int cont;
